...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        for j in xrange(epochs):
            ...
            for mini_batch in mini_batches:
                self.update_mini_batch(mini_batch, eta) #训练，eta为学习率
            if test_data:
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        for x, y in mini_batch:
            delta_nabla_b, delta_nabla_w = self.backprop(x, y) #反向传播，逐层计算b,w的梯度
        ...
        for b, w in zip(self.biases, self.weights):  #向进行一次前向传播
            z = np.dot(w, activation)+b #权值更新
            zs.append(z)
            activation = sigmoid(z) #更新神经元输出，sigmoid激活函数
